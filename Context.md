...
**대용량 데이터셋 테스트 (2025-10-22):**
- ✅ **재귀 스캔 테스트**: 네트워크 볼륨 전체 스캔
  - 실행 시간: **3분 36.86초** (216.86초)
  - CPU 사용률: 15% (네트워크 I/O bound)
  - User time: 15.44초 / System time: 18.82초
  - 병렬 처리 및 스트리밍 파이프라인 정상 작동
  - 수백 개 게임 정상 탐지 및 처리
- ✅ 성능 특성:
  - 낮은 CPU 사용률 → 네트워크 I/O가 주요 병목
  - 병렬 스캔이 효과적으로 I/O 대기 시간 활용
  - 메모리 사용량 안정적 유지
